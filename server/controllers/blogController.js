...
exports.create = async (req, res) => {
  try {
    const {
      title, slug, excerpt, metaDescription, content,
      coverImage, category, tags, published,
      status, scheduledAt, aiGenerated, aiTone,
    } = req.body;
    if (!title || !content) return res.status(400).json({ message: 'title and content are required' });

    const finalStatus = status || (published === true || published === 'true' ? 'published' : 'draft');

    const baseSlug = slug || title.toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-').slice(0, 80);
    ...
    const post = await Blog.create({
      title, slug: finalSlug, excerpt,
      metaDescription: metaDescription || '',
      content, coverImage, category,
    ...
    console.error('Blog create error:', e.message, e.errors);
...
exports.update = async (req, res) => {
  try {
    const {
      title, slug, excerpt, metaDescription, content,
      coverImage, category, tags, published, status, scheduledAt,
    } = req.body;

    const finalStatus = status || (published === true || published === 'true' ? 'published' : undefined);
    const update = {
      title, slug, excerpt, metaDescription, content,
      coverImage, category,
    ...
    if (finalStatus !== undefined) {
      ...
      update.published = finalStatus === 'published';
    }
