  ...
  return (
    <div className="w-full bg-[var(--color-page)] pb-20 lg:pb-0">
          ...
          <div>
            <div className="relative overflow-hidden rounded-2xl border border-[var(--color-border)] bg-white shadow-md group cursor-zoom-in"
              ...
              <img src={assetUrl(images[imgIdx])} alt={product.name}
                className="aspect-[4/3] md:aspect-square w-full object-contain bg-gray-50 transition-transform duration-500 group-hover:scale-105" />
              {product.discount > 0 && (
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-5">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-cyan-50 to-blue-50 border border-cyan-100 p-5">
              <div className="flex flex-wrap items-end gap-2 mb-2">
                <span className="text-2xl md:text-3xl font-bold" style={{ color: "#00A6C4" }}>{fmt(effective)}</span>
                {product.discount > 0 && (
                  <>
      ...
      <div className="px-4 md:px-6 lg:px-12 py-8 border-t border-[var(--color-border)]">
        <div className="flex gap-0 mb-6 border-b border-[var(--color-border)] overflow-x-auto"
          ...
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={"flex-shrink-0 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition whitespace-nowrap " + (tab === t.id
