...
export function Login() {
  ...
  async function handleGoogleSuccess(credentialResponse) {
    try {
      ...
      const hasDraft = !!sessionStorage.getItem('coldtech_service_draft');
      if (loggedInUser.role === 'admin' || loggedInUser.role === 'staff') {
        navigate('/admin-coldtech-secure', { replace: true });
      } else if (hasDraft) {
        navigate('/services/request', { replace: true });
      } else {
  ...
  async function submit(e) {
    ...
    try {
      ...
      // If there's a pending service draft, go back to service request to auto-submit
      const hasDraft = !!sessionStorage.getItem('coldtech_service_draft');
      if (loggedInUser?.role === 'admin' || loggedInUser?.role === 'staff') {
        navigate("/admin-coldtech-secure", { replace: true });
      } else if (hasDraft) {
